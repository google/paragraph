...
#ifndef PARAGRAPH_SHIM_STATUSOR_INTERNALS_H_
#define PARAGRAPH_SHIM_STATUSOR_INTERNALS_H_

#include <utility>

#include "absl/status/status.h"

#define PARAGRAPH_SHIM_ABSL_ATTRIBUTE_NORETURN __attribute__((noreturn))
...
class StatusOrData {
 ...
 protected:
  ...
  void EnsureOk() const {
    if (!this->ok()) {
      const char* kMessage =
          "Failed retrieve value from StatusOr";
      fprintf(stderr, "%s %s\n", kMessage, status_.ToString().c_str());
      abort();
    }
  }
  ...
  void EnsureNotOk() {
    if (this->ok()) {
      const char* kMessage =
          "An OK status is not a valid constructor argument to StatusOr\n";
      fprintf(stderr, "%s", kMessage);
      status_ = absl::Status(absl::StatusCode::kInternal, (kMessage));
      abort();
    }
  }
...
#endif  // PARAGRAPH_SHIM_STATUSOR_INTERNALS_H_
