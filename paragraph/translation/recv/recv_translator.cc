...
absl::Status RecvTranslator::Translate(Instruction* instruction) const {
...
shim::StatusOr<std::unique_ptr<RecvTranslator>>
    RecvTranslator::Create(nlohmann::json config) {
  const std::string& algorithm = config["algorithm"].get<std::string>();
  std::unique_ptr<RecvTranslator> translator = absl::WrapUnique(
      factory::ObjectFactory<RecvTranslator, nlohmann::json>::create(
          algorithm, config));
  RETURN_IF_FALSE(translator != nullptr, absl::InvalidArgumentError) <<
      "Recv algorithm name " << config["algorithm"] <<
