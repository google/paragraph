 ...
 */
#ifndef PARAGRAPH_TRANSLATION_ALLGATHER_MESH_2D_ALLGATHER_TRANSLATOR_H_
#define PARAGRAPH_TRANSLATION_ALLGATHER_MESH_2D_ALLGATHER_TRANSLATOR_H_
...
/* Mesh2dAllGatherTranslator models a all-gather collective on the
 ...
 * algorithm splits all-gather into two phases. First it gathers data from
 * the processors across the first dimentsion of size M. Second it gathers
 * partially gathered data across the 2nd dimenstion. In case mesh has
 ...
 */
class Mesh2dAllGatherTranslator : public AllGatherTranslator {
 public:
  explicit Mesh2dAllGatherTranslator(nlohmann::json config);
  ~Mesh2dAllGatherTranslator() = default;
...
#endif  // PARAGRAPH_TRANSLATION_ALLGATHER_MESH_2D_ALLGATHER_TRANSLATOR_H_
