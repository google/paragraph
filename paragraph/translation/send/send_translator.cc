...
absl::Status SendTranslator::Translate(Instruction* instruction) const {
...
shim::StatusOr<std::unique_ptr<SendTranslator>>
    SendTranslator::Create(nlohmann::json config) {
  const std::string& algorithm = config["algorithm"].get<std::string>();
  std::unique_ptr<SendTranslator> translator = absl::WrapUnique(
      factory::ObjectFactory<SendTranslator, nlohmann::json>::create(
          algorithm, config));
  RETURN_IF_FALSE(translator != nullptr, absl::InvalidArgumentError) <<
      "Send algorithm name " << config["algorithm"] <<
