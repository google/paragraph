 ...
 */
#ifndef PARAGRAPH_TRANSLATION_SENDRECV_SENDRECV_TRANSLATOR_H_
#define PARAGRAPH_TRANSLATION_SENDRECV_SENDRECV_TRANSLATOR_H_
...
class SendRecvTranslator : public Translator {
 public:
  virtual ~SendRecvTranslator() = default;

  // Creates particular SendRecv algorithm based on Json config
  static shim::StatusOr<std::unique_ptr<SendRecvTranslator>>
  ...
  virtual shim::StatusOr<std::unique_ptr<Subroutine>> GetSubroutine(
      ...
      double comm_Size_recv) const = 0;
};

}  // namespace paragraph

#endif  // PARAGRAPH_TRANSLATION_SENDRECV_SENDRECV_TRANSLATOR_H_
