...
shim::StatusOr<std::unique_ptr<Subroutine>>
  ...
  for (size_t dim = 0; dim < dimension_sizes_.size(); dim++) {
    ...
    if (comm_group_torus.size() > 1) {
      ASSIGN_OR_RETURN(auto reducescatter_torus, Instruction::Create(
          Opcode::kReduceScatter,
          absl::StrCat(name_prefix, "_dim-", dim),
          reducescatter_sub_ptr));
      reducescatter_torus->AppendCommunicationGroup(comm_group_torus);
      reducescatter_torus->SetBytesOut(comm_size * dim_width /
                                      comm_group.size());
      ASSIGN_OR_RETURN(auto reduction_subroutine_torus,
                       reduction_subroutine->Clone("", /*reset_ids*/ false));
      reduction_subroutine_torus->ScalePerformance(1.0 * dim_width
                                                  / comm_group.size());
      reducescatter_torus->AppendInnerSubroutine(std::move(
          reduction_subroutine_torus));
      if (previous_instruction != nullptr) {
        reducescatter_torus->AddOperand(previous_instruction);
      }
      RETURN_IF_ERROR(reducescatter_translator_->Translate(
          reducescatter_torus));
      previous_instruction = reducescatter_torus;
    }
  }
