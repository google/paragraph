...
absl::Status ReduceScatterTranslator::Translate(
...
shim::StatusOr<std::unique_ptr<ReduceScatterTranslator>>
    ReduceScatterTranslator::Create(nlohmann::json config) {
  const std::string& algorithm = config["algorithm"].get<std::string>();
  std::unique_ptr<ReduceScatterTranslator> translator = absl::WrapUnique(
      factory::ObjectFactory<ReduceScatterTranslator, nlohmann::json>::create(
          algorithm, config));
  RETURN_IF_FALSE(translator != nullptr, absl::InvalidArgumentError) <<
      "Reduce-Scatter algorithm name " << config["algorithm"] <<
