...
absl::Status BarrierTranslator::Translate(Instruction* instruction) const {
...
shim::StatusOr<std::unique_ptr<BarrierTranslator>>
    BarrierTranslator::Create(nlohmann::json config) {
  const std::string& algorithm = config["algorithm"].get<std::string>();
  std::unique_ptr<BarrierTranslator> translator = absl::WrapUnique(
      factory::ObjectFactory<BarrierTranslator, nlohmann::json>::create(
          algorithm, config));
  RETURN_IF_FALSE(translator != nullptr, absl::InvalidArgumentError) <<
      "Barrier algorithm name " << config["algorithm"] <<
